      ...
      <v-row justify="center">
        <v-form ref="form" v-model="valid" lazy-validation class="justify:center">
          ...
          <v-text-field v-model="password" label="Password" :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'"
            :type="show1 ? 'text' : 'password'" :rules="passwordRules" name="Password" hint="At least 8 characters"
            counter required @click:append="show2 = !show2"></v-text-field>
        </v-form>
      </v-row>
      <v-row justify="center">
        <v-btn color="teal lighten-3 white--text" class="mr-4" @click="login">Iniciar sesión</v-btn>
      </v-row>
